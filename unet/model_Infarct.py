...
import numpy as np
from unet.function import readmat
import cv2
    ...
    def train(self, train_images, train_lanbels, model_path, logs_path, learning_rate,
        ...
        for i in range(train_epochs):
            ...
            for num in range(len(batch_xs_path)):
                ...
                image = readmat(batch_xs_path[num][0],'multimodalitydata')
                label = readmat(batch_ys_path[num][0], 'multimodalitymask')
            ...
            if i % DISPLAY_STEP == 0 or (i + 1) == train_epochs:
                ...
                # cv2.imwrite("result.bmp", result)#存储每一次的预测结果
